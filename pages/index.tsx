...
interface HomeProps {
    ...
    initialForecast: ForecastDataList[];
    initialCity: string;
}

export const getServerSideProps: GetServerSideProps<HomeProps> = async (context) => {
    const defaultCity = 'Москва';
    let initialWeatherData: WeatherData | null = null;
    let initialForecast: ForecastDataList[] = [];
    const cookies = context.req.headers.cookie.match(/i18next=([^;]+)/)[1]
    console.log(cookies)

    try {
        const lang =  await cookies || 'ru';
        initialWeatherData = await weatherAPI.getWeatherData(defaultCity, lang);
        const forecastData = await weatherAPI.getForecastData(defaultCity, lang);
        initialForecast = forecastData.list.filter((_, index) => index % 8 === 0);
    } catch (err) {
...
export default function Home({ initialWeatherData, initialForecast, initialCity }: HomeProps) {
    const [isFetching, setIsFetching] = useState<boolean>(false);
    const [searchInput, setSearchInput] = useState<string>('');
    const [weatherData, setWeatherData] = useState<WeatherData | null>(initialWeatherData);
    const [city, setCity] = useState<string>(initialCity);
    const [forecast, setForecast] = useState<ForecastDataList[]>(initialForecast);
                                            ...
                                            <div className={s.detailsRow}>
                                                <div className={s.detailsDuo}>
                                                    ...
                                                    <div className={s.detailItem}>
                                                        <SpeedWind />
                                                        <p>
                                                            {Math.round(weatherData.wind.speed)} {t('weatherPage.m/s')}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className={s.detailsDuo}>
